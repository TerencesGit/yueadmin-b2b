	...
	<section>
		...
		<el-table :data="orderList" border highlight-current-row v-loading="loading">
			...
			<el-table-column label="操作" width="160">
				<template scope="scope">
					<el-button size="small" @click="handleCompletion(scope.$index, scope.row)">完善信息</el-button>
				</template>
			</el-table-column>
		</el-table>
		<el-row class="toolbar">
...
<script>
	import { readOrderList , pendingPay} from '@/api'
	export default {
		data() {
			return {
				loading: false,
		...
		methods: {
			...
			handleCompletion (index,row) {
				this.$router.push({
					path:'/distributor/order/customInfoCompletion',
					query: { wareName: row.wareName,adultCount:row.adultCount,orderId:row.orderId }
				})
			},
			// 订单列表
			getOrderList() {
