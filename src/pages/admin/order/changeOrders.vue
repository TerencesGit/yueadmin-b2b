<template>
    <section>
        ...
        <el-table :data="orderList" border highlight-current-row v-loading="loading">
            ...
            <el-table-column prop="statusComplain" label="订单状态" width="120" :formatter="formatStatus"></el-table-column>
            ...
            <el-table-column label="操作" width="160">
                <template scope="scope">		    		
			        <el-button size="small" @click="switchBill(scope.$index, scope.row)">改单</el-button>
			        <el-button size="small" @click="handleCheck(scope.$index, scope.row)">查看</el-button>
        		</template>
            </el-table-column>
        </el-table>
        <el-row class="toolbar page">
            <el-pagination
...
<script>
    ...
    export default {
        ...
        methods: {
	        ...
	        handleCheck (index,row) {
				this.$router.push({
					path:'/admin/order/changeOrderDetail',
					query: { orderId: row.orderId }
				})
			},
        	switchBill(index,row){
        		this.$router.push({
        			path:'/admin/order/changeOperation',
        			query: { orderId: row.orderId,wareId:row.wareId }
        		})
        	},
        // 订单列表
            getOrderList() {
