<template>
  <section>
	...
	<el-table :data="orderList" border highlight-current-row v-loading="loading">
	    ...
	    <el-table-column prop="statusRefund" label="订单状态" width="120" :formatter="formatStatus"></el-table-column>
	...
	export default {
		...
		methods: {
			...
			handleCheck (index,row) {
				this.$router.push({
					path:'/admin/order/refundOrderDetail',
					query: { orderId: row.orderId }
				})
			},
			refundApprove(index,row){
				this.$router.push({
					path:'/admin/order/refundApplyingDetail',
					query: { orderId: row.orderId }
				})
			},
			formatStatus (row) {
